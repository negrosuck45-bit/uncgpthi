...
const KEYWORDS: Record<string, string[]> = {
  python: ['def', 'class', 'import', 'from', 'return', 'if', 'else', 'elif', 'for', 'while', 'try', 'except', 'finally', 'with', 'as', 'lambda', 'yield', 'async', 'await', 'print', 'len', 'range', 'list', 'dict', 'set', 'tuple', 'str', 'int', 'float', 'bool', 'None', 'True', 'False', 'and', 'or', 'not', 'in', 'is', 'pass', 'break', 'continue', 'raise', 'assert', 'del', 'global', 'nonlocal'],
  javascript: ['const', 'let', 'var', 'function', 'class', 'extends', 'import', 'export', 'default', 'return', 'if', 'else', 'for', 'while', 'do', 'switch', 'case', 'break', 'continue', 'try', 'catch', 'finally', 'throw', 'new', 'this', 'typeof', 'instanceof', 'void', 'delete', 'in', 'of', 'await', 'async', 'yield', 'debugger', 'true', 'false', 'null', 'undefined', 'NaN', 'Infinity'],
  bash: ['if', 'then', 'else', 'elif', 'fi', 'for', 'while', 'do', 'done', 'case', 'esac', 'function', 'return', 'exit', 'echo', 'printf', 'read', 'source', 'export', 'unset', 'local', 'declare', 'typeset', 'readonly', 'shift', 'break', 'continue', 'test', '[', ']]', 'true', 'false']
};
...
function highlightCode(code: string, language: string): string {
  ...
  highlighted = highlighted.replace(/('(?:[^'\]|\.)*')/g, `<span style="color:${colors.string}">$1</span>`);
  if (language === 'python') {
    highlighted = highlighted.replace(/("""[\s\S]*?""")/g, `<span style="color:${colors.string}">$1</span>`);
  }

  // Numbers
  highlighted = highlighted.replace(/(\d+\.?\d*)/g, `<span style="color:${colors.number}">$1</span>`);
  ...
  // Functions
  highlighted = highlighted.replace(/(\w+)(?=\()/g, `<span style="color:${colors.function}">$1</span>`);
...
function formatOutput(output: string, language: string): React.ReactNode {
  ...
  return lines.map((line, i) => {
    ...
    if (line.includes('Warning') || line.includes('WARN') || line.includes('warning')) {
      return (
        <div key={i} className="text-yellow-400 font-mono text-sm py-0.5 px-2 hover:bg-yellow-950/30 rounded">
          <span className="text-yellow-500 mr-2">⚠</span>{line}
        </div>
      );
    }
    // Success lines
    ...
    // Info/Log lines
    if (line.includes('INFO') || line.includes('log') || line.includes('[INFO]')) {
      return (
        <div key={i} className="text-cyan-400 font-mono text-sm py-0.5 px-2 hover:bg-cyan-950/30 rounded">
          <span className="text-cyan-500 mr-2">ℹ</span>{line}
        </div>
      );
    }
