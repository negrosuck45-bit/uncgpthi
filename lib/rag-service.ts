...
): Promise<{ success: boolean; documentId?: string; chunks?: number; error?: string }> {
  try {
...
async function extractTextFromPDF(arrayBuffer: ArrayBuffer): Promise<string> {
  ...
  // For now, return a placeholder that works with text files too
  ...
  // For actual PDFs, you'd need pdf-parse library
...
// Get context for chat (top chunks combined)
export async function getRAGContext(query: string, userId: string, maxTokens: number = 2000): Promise<string> {
  ...
  for (const result of results) {
    ...
    if (currentLength + chunk.length > maxTokens * 4) break; // Rough token estimate
