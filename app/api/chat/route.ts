...
export async function POST(req: NextRequest) {
  try {
    ...
    const systemParts: string[] = [
      `You are uncgpt — a helpful, conversational AI assistant. You can also use tools when needed.\n\n# HOW TO BEHAVE\n- For greetings ("hi", "hello", "how are you"), questions, opinions, explanations, creative writing, jokes, general chat — JUST TALK NORMALLY. Be friendly, natural, and conversational. NO tools needed.\n- ONLY use tools when the user explicitly asks you to DO something:\n  * "run this code" / "execute" → use terminal/code tools\n  * "create a GitHub repo" / "push file" → use GitHub tools\n  * "send Slack message" → use Slack tools\n  * "search the web" → use web search\n  * "what's in my PDF" → use RAG/document search\n  * "list my Linear issues" → use Linear tools\n\n# IF A CONNECTOR IS NOT CONNECTED\nReply: "I can't do that yet — click the 'Add connectors' button and connect <SERVICE>, then ask me again."\n\n# REPLY STYLE\nAfter tool results: 1-2 line summary + the returned URL/value. No filler.`,
    ];
    ...
    try {
      ...
      const availableNames = [
        ...BUILTIN_TOOLS.map((t: any) => t.function.name),
        ...oauthBundle.tools.map((t: any) => t.function.name),
        ...mcpTools.map((t: any) => t.function.name),
      ];
      ...
      if (availableNames.length > 0) {
        ...
        messagesWithSystem[0] = {
          ...
          content:
            ...
            `\n\n# CONNECTED: ${oauthBundle.connected.join(", ") || "none"}` +
            `\n# NOT CONNECTED: ${notConnected.join(", ") || "none"}` +
            `\n\n# RULE: Only call tools for explicit action requests. For normal chat, just respond with text.`,
        };
    ...
    console.log(`[UNCGPT] Model: ${result.model} | Provider: ${result.provider} | Tools: ${toolSteps.length}`);
